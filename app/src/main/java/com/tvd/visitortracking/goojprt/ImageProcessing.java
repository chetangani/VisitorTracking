...
public class ImageProcessing {
    ...
    public static byte[] eachLinePixToCmd(boolean[] src, int nWidth, int nMode) {
        ...
        for(int i = 0; i < nHeight; ++i) {
            ...
            for(int j = 0; j < nBytesPerLine; ++j) {
                data[var10 + 8 + j] = (byte)((src[k]?128:0) | (src[k + 1]?64:0) | (src[k + 2]?32:0) | (src[k + 3]?16:0) | (src[k + 4]?8:0) | (src[k + 5]?4:0) | (src[k + 6]?2:0) | (src[k + 7]?1:0));
                k += 8;
            }
        }
    ...
    public static byte[] eachLinePixToCompressCmd(boolean[] src, int nWidth) {
        ...
        for(compresseddatalen = 0; compresseddatalen < nHeight; ++compresseddatalen) {
            for(compresseddatabytes = 0; compresseddatabytes < nBytesPerLine; ++compresseddatabytes) {
                data[compresseddatalen * nBytesPerLine + compresseddatabytes] = (byte)((src[k]?128:0) | (src[k + 1]?64:0) | (src[k + 2]?32:0) | (src[k + 3]?16:0) | (src[k + 4]?8:0) | (src[k + 5]?4:0) | (src[k + 6]?2:0) | (src[k + 7]?1:0));
                k += 8;
            }
        }
        ...
        byte[] line;
        for(compresseddatabytes = 0; compresseddatabytes < nHeight; ++compresseddatabytes) {
            byte[] offset = new byte[nBytesPerLine];
            System.arraycopy(data, compresseddatabytes * nBytesPerLine, offset, 0, nBytesPerLine);
            byte[] y = CompressDataBuf(offset);
            line = new byte[]{31, 40, 80, (byte)((int)((long)y.length & 255L)), (byte)((int)(((long)y.length & 65535L) >> 8))};
            compresseddatalen += line.length;
            compresseddatalen += y.length;
        }

        byte[] var13 = new byte[compresseddatalen];
        int var14 = 0;

        for(int var15 = 0; var15 < nHeight; ++var15) {
            line = new byte[nBytesPerLine];
            System.arraycopy(data, var15 * nBytesPerLine, line, 0, nBytesPerLine);
            byte[] buf = CompressDataBuf(line);
            byte[] cmd = new byte[]{31, 40, 80, (byte)((int)((long)buf.length & 255L)), (byte)((int)(((long)buf.length & 65535L) >> 8))};
            System.arraycopy(cmd, 0, var13, var14, cmd.length);
            var14 += cmd.length;
            System.arraycopy(buf, 0, var13, var14, buf.length);
            var14 += buf.length;
        }
    ...
    static int ThreeOrder0(int srcw, int srch, int[] src, double fx, double fy) {
        ...
        for(int afu = 0; afu < 4; ++afu) {
            for(int afv = 0; afv < 4; ++afv) {
                long sR = (long)(x0 - 1 + afv);
                long sB = (long)(y0 - 1 + afu);
                pixel[afu * 4 + afv] = Pixels_Bound(srcw, srch, src, sR, sB);
            }
        }

        double[] var26 = new double[4];
        double[] var27 = new double[4];
        var26[0] = SinXDivX(1.0D + fu);
        var26[1] = SinXDivX(fu);
        var26[2] = SinXDivX(1.0D - fu);
        var26[3] = SinXDivX(2.0D - fu);
        var27[0] = SinXDivX(1.0D + fv);
        var27[1] = SinXDivX(fv);
        var27[2] = SinXDivX(1.0D - fv);
        var27[3] = SinXDivX(2.0D - fv);
        float var28 = 0.0F;
        float sG = 0.0F;
        float var29 = 0.0F;
        float sA = 0.0F;
        ...
        for(int a = 0; a < 4; ++a) {
            ...
            for(int j = 0; j < 4; ++j) {
                aA = (float)((double)aA + var26[j] * (double)pixel[a * 4 + j].a);
                r = (float)((double)r + var26[j] * (double)pixel[a * 4 + j].r);
                g = (float)((double)g + var26[j] * (double)pixel[a * 4 + j].g);
                b = (float)((double)b + var26[j] * (double)pixel[a * 4 + j].b);
            }

            sA = (float)((double)sA + (double)aA * var27[a]);
            var28 = (float)((double)var28 + (double)r * var27[a]);
            sG = (float)((double)sG + (double)g * var27[a]);
            var29 = (float)((double)var29 + (double)b * var27[a]);
        }

        byte var30 = (byte)((int)border_color((long)((double)sA + 0.5D)));
        byte var31 = (byte)((int)border_color((long)((double)var28 + 0.5D)));
        byte var32 = (byte)((int)border_color((long)((double)sG + 0.5D)));
        byte var33 = (byte)((int)border_color((long)((double)var29 + 0.5D)));
        return (int)(((long)var30 & 255L) << 24 | ((long)var31 & 255L) << 16 | ((long)var32 & 255L) << 8 | ((long)var33 & 255L) << 0);
    }
    ...
    static double SinXDivX(double x) {
        ...
        double x2 = x * x;
        double x3 = x2 * x;
        return x <= 1.0D?1.0D * x3 - 2.0D * x2 + 1.0D:(x <= 2.0D?-1.0D * x3 - -5.0D * x2 + -8.0D * x - -4.0D:0.0D);
    }

    static long border_color(long Color) {
        return Color <= 0L?0L:(Color >= 255L?255L:Color);
    }
